...
import {StarFill, Trash2} from "react-bootstrap-icons";
import AdminEditProduct from "./AdminEditProduct";

const AdminProductsCards = ({product: {_id, imageCover, title, rattingAverage, description}}) => {
    const {deleteHandler} = DestroyProduct(_id)

    return (
        <tr>
            <th><img onError={(e) => e.target.src = image}
                     src={imageCover} alt={title} width={20}/></th>
            <th>{title}{rattingAverage && (<>{rattingAverage} <StarFill size={15}
                                                                        className="text-warning"/></>)}</th>

...
export const CreateProduct = () => {
  ...
  const { create, create_error, loading } = useSelector(
    (state) => state.products
  );
...
export const EditProduct = (id) => {
  ...
  const { categories } = GetCategories();
  const { brands } = GetBrands();
  const { edit, edit_error, loading } = useSelector((state) => state.products);
  const { subcategoriesByCategory } = useSelector(
    (state) => state.subcategories
  );
  ...
  }, [data.category, data.subCategory, dispatch, subcategoriesByCategory.data]);
  // Add color to the colors array
  useEffect(() => {
    if (subcategoriesByCategory?.data && data.subCategory) {
      const selectedOptions = subcategoriesByCategory.data.filter(
        (subcategory) => data.subCategory.includes(subcategory._id)
      );
      setSelectedSubcategoryOptions(selectedOptions);
    }
  }, [data.subCategory, subcategoriesByCategory.data]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const { title, description, quantity, price, category, brand } = data;
    const formData = new FormData();
    let imgCover;
  ...
  useEffect(() => {
    ...
    if (!loading && edit.status === 200 && isPress) {
      setIsPress(false);
      use_notification("The product has been updated successfully!", "success");
    }
...
export const DestroyProduct = (id) => {
  ...
  const currentPage = useSelector(
    (state) => state.products.products.paginationResult?.currentPage
  );
